#first_Experiment.R
#809 holdiday vs 1000 (random) Person
library(reshape2)
library(h2o)
...
personCases_selection <- personCases[sample(x = nrow(personCases), size = 1000, replace = FALSE),]
...
types <- rbind(holidayCases,personCases_selection)
properties <- rbind(holidayProperties,personProperties)
...
rf_model_Holiday_vs_Person <- h2o.randomForest(
  model_id="rf_model_Holiday_vs_Person",
  ...
  seed = 1234)
h2o.saveModel(rf_model_Holiday_vs_Person, path=paste(getwc(),"/someModelsHoliday/",sep=""))
